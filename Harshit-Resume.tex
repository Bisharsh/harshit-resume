...
% DevOps Engineer with 3+ years of hands-on experience building and operating cloud-native platforms. Strong in Kubernetes, CI/CD \& infra automation, with a focus on reliability, security \& developer experience. Proven at reducing operational bottlenecks, supporting release strategies \& owning scalable production systems.
DevOps Engineer with 3+ years of hands-on experience designing \& operating Kubernetes, helmcharts, CI/CD and automated cloud infrastructure. Strong focus on reliability, security \& developer experience with proven impact in reducing operational bottlenecks, enabling release strategies and owning scalable production systems.
% Passionate DevOps Engineer with 3+ years of experience specializing in automation, cloud infrastructure \& kubernetes. Proven track record of reducing CI/CD bottlenecks, secret management, optimizing k8s clusters \& deployments, writing clear documentations, designing resilient CI/CD pipelines and reducing downtime.
...
\end{itemize}
\begin{dashlist}
    \item \uline{Multiple parallel app versions}: Configured Istio, CI/CD \& helmcharts to enable deployment of multiple app versions, supporting complex release strategies (four versions in parallel instead of blue-green).
    \item \uline{provisioned robust cloud infra}: Ensured high availability \& security by provisioning Azure infrastructure using Terraform modules like Azure APIM, Managed identify \& security groups.
    \item \uline{PR enhancement workflow}: Introduced a github actions workflow to provide soft guidance to dev/reviewer via comments on PRs, based on the conventional commit types.
    \item \uline{k8s lease}: Implemented a Kubernetes Lease-based leader election mechanism to avoid multiple cronjob runs. 
    \item \uline{Helm Chart segregation}: Improved CI/CD by separating helmcharts like: VS, DR, SA, roles; from app-specific charts, reducing re-rendering of non app-specific charts \& noise in ACR.
    \item \uline{Introduced e2e testing}: Designed \& implemented end-to-end testing automation the cd \& release AzDO pipeline, writing jest based tests.
\end{dashlist}
\medskip
...
\begin{dashlist}
    ...
    \item \uline {Multi-region AI infrastructure management}: Provisioned \& maintained Azure AI infrastructure like: document intelligence for ChatPwC across EMEA \& US using Terraform modules \& Azure DevOps.
...
\begin{dashlist}
    ...
    \item \uline {Logistics Optimization}: Supported logistics operations for Amazon Logistics across the US and Canada.
    ...
    \item \uline {KT \& Team lead}: Served as a Subject Matter Expert and mentored 8 new associates on cloud-based logistics data management.
    ...
    \item \uline {Quality improvement initiative}: Reviewed 100+ weekly work items and provided targeted feedback, reducing errors by 20\% \& improving overall performance.
\end{dashlist}
...
\end{itemize}
\begin{dashlist}
    \item Designed an AzDO pipeline to automate secrets sync from Azure Library to Azure Key Vault.
    \item Implemented object-based parameter storage, enabling retrieval of both secret names and values.
...
\end{itemize}
\begin{dashlist}
    \item Provisioned and managed AWS infrastructure using Crossplane.
...
\end{itemize}
\begin{dashlist}
    \item Automated AWS infra provisioning using modular Terraform and GitHub Actions, leveraging S3.
    \item Built CI/CD with GitHub Actions, Kaniko \& ArgoCD, eliminating AWS access keys via OIDC.
...
\end{itemize}
\begin{dashlist}
    \item Deployed a Kubernetes cluster on GKE using Terraform, implementing taints, tolerations, and affinities.
    \item Configured custom VPC \& subnets for pods. Used Google Cloud Build for deployments to GKE.
...
\begin{flushright} % Aligns text to the right
    \textit{Date: 11.02.2025}\\ % Adds the date below the name
